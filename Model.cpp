    ...
    if (obj.is_open())
    {
        while (getline(obj, input))
        {
            ...
            if (split[0] == "v")
            {
                ...
                position_vector.push_back(getFloat(split[4]));
            }
            else if (split[0] == "vt")
            {
...
void Model::Draw()
{
    ...
    for (int i = 0; i < sz; i++)
    {
        ...
        glVertex3f(position_texture_normal[index]*2, position_texture_normal[index + 1]*2, position_texture_normal[index + 2]*2);
    }
    glEnd();
